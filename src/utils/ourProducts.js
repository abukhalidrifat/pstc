export const ourProducts = [
  {
    title: "Power Generation System",
    isSubTitled: false,
    sub: [
      {
        name: "Gas Turbine Generator",
      },
      {
        name: "Steam Turbine Generator",
      },
      {
        name: "Heat Recovery Steam Generator (HRSG)",
      },
      {
        name: "Boiler",
      },
      {
        name: "Natural Gas Generator",
      },
      {
        name: "Diesel Generator",
      },
      {
        name: "HFO Generator",
      },
      {
        name: "Gasoline Generator",
      },
      {
        name: "Air Compressor",
      },
      {
        name: "Air Dryer",
      },
      {
        name: "Spare Parts",
      },
    ],
  },
  {
    title: "Power Transmission & Distribution Substation System",
    isSubTitled: true,
    sub: [
      {
        subTitle: "Low Voltage Products & Systems",
        subOfSub: [
          {
            name: "MCB",
          },
          {
            name: "MCCB",
          },
          {
            name: "RCCB",
          },
          {
            name: "Air Circuit Breaker (ACB)",
          },
          {
            name: "Surge Protective DeviceSurge Protective Device",
          },
          {
            name: "Magnetic Contactor",
          },
          {
            name: "Thermal Overload Relay",
          },
          {
            name: "Contactor for capacitor switching",
          },
          {
            name: "Power Supply Capacitor",
          },
          {
            name: "Power Factor Correction Controller",
          },
          {
            name: "Fuse Link. Fuse Gear",
          },
          {
            name: "General Purpose Relay",
          },
          {
            name: "Automatic Transfer Switch (ATS)",
          },
          {
            name: "Changeover Switch",
          },
          {
            name: "Changeover Switch",
          },
          {
            name: "Switch and Indicator Lamp",
          },
          {
            name: "Current Transformer (LV)",
          },
          {
            name: "Power Cable and Control Cable",
          },
          {
            name: "Spare Parts",
          },
        ],
      },
      {
        subTitle: "Medium Voltage Products & Systems",
        subOfSub: [
          {
            name: "MV Vacuum Circuit Breaker",
          },
          {
            name: "MV SF6 Gas Circuit Breaker",
          },
          {
            name: "MV Vacuum Contactor",
          },
          {
            name: "MV Load Break Switch",
          },
          {
            name: "MV Load Break Switch",
          },
          {
            name: "MV Outdoor Vacuum Recloser",
          },
          {
            name: "MV Indoor/Outdoor Dry Type/Oil Filled Current Transformer",
          },
          {
            name: "MV Indoor/Outdoor Dry Type/Oil Filled Voltage Transformer",
          },
          {
            name: "MV Switches, Disconnector & Isolator",
          },
          {
            name: "MV Earthing Switch",
          },
          {
            name: "MV Surge Arrester (Indoor & Outdoor)",
          },
          {
            name: "MV Fuse",
          },
          {
            name: "MV Fuse Cutout",
          },
          {
            name: "MV Insulator",
          },
          {
            name: "MV Heat Shrink",
          },
          {
            name: "MV Power Cable and Wire",
          },
          {
            name: "Spring Operating Mechanism  for LBS",
          },
          {
            name: "Electromagnet Mechanism for LBS",
          },
          {
            name: "Manual Mechanism for LBS",
          },
          {
            name: "Operation Mechanism for VCB",
          },
          {
            name: "Gear Motor for VCB",
          },
          {
            name: "Opening Coil & Closing Coil for VCB",
          },
          {
            name: "Contactor Head for VCB",
          },
          {
            name: "Female Contactor Box",
          },
          {
            name: "Male Contactor Box",
          },
          {
            name: "Sealing Pole for VCB",
          },
          {
            name: "Gas Insulating Enclosure",
          },
          {
            name: "VCB Truck",
          },
          {
            name: "VCB Gear",
          },
          {
            name: "VCB Socket",
          },
          {
            name: "Vacuum Interrupter for VCB",
          },
          {
            name: "Vacuum Interrupter for LBS ",
          },
          {
            name: "Vacuum Interrupter for contactor, Recloser & sectional switch",
          },
          {
            name: "Switchgear Enclosure",
          },
          {
            name: "VCB Complete Parts",
          },
          {
            name: "LBS Complete Parts",
          },
        ],
      },
      {
        subTitle: "High Voltage Products & Systems",
        subOfSub: [
          {
            name: "HV Generator Circuit Breaker",
          },
          {
            name: "HV Disconnector",
          },
          {
            name: "HV Oil Filled Current Transformer",
          },
          {
            name: "HV Oil Filled Capacitive/Inductive Voltage Transformer",
          },
          {
            name: "HV Live Tank/Dead Tank SF6 Circuit Breaker",
          },
          {
            name: "HV Surge Arrester",
          },
          {
            name: "HV Insulator",
          },
          {
            name: "HV Overhead Line and accessories",
          },
          {
            name: "HV Overhead Line Steel Tower",
          },
          {
            name: "HV Underground Cable",
          },
          {
            name: "Spare Parts",
          },
        ],
      },
      {
        subTitle: "Transformer",
        subOfSub: [
          {
            name: "Step up Oil Immersed Power Transformer (11KV/33KV/132 KV/230KV)",
          },
          {
            name: "Step down Oil Immersed Power Transformer (132 KV/33 KV/11 KV)",
          },
          {
            name: "Distribution Transformer (11 KV/0.4KV,6.6KV/0.4KV,3.3 KV/0.4KV)",
          },
          {
            name: "Dry Type Transformer (11KV/0.4KV, 6.6KV/0.4KV)",
          },
          {
            name: "Transformer Electrician Pressing Parts",
          },
          {
            name: "High Low Voltage Conductor Pole",
          },
          {
            name: "Thermolysis Plates",
          },
          {
            name: "Connection Joint End",
          },
          {
            name: "Substation Conductor",
          },
          {
            name: "Cold Press Connection Thrum",
          },
          {
            name: "Oil Discharge Valve",
          },
          {
            name: "Transformer Cup",
          },
          {
            name: "Conductor",
          },
          {
            name: "On Load Tap Changer",
          },
          {
            name: "Offload Tap Changer",
          },
          {
            name: "Oil Level Indicator",
          },
          {
            name: "Transformer Enclosure",
          },
          {
            name: "Transformer Conductor Plate",
          },
          {
            name: "Ripple Plates",
          },
          {
            name: "Composite High Voltage Conduct Pipe for fully sealed transformer",
          },
          {
            name: "Conduct Pipe For Fully sealed Transformer",
          },
          {
            name: "Conductor Pipe for fully sealed transformer",
          },
          {
            name: "Composite Insulator for dry type transformer",
          },
          {
            name: "Transformer Oil Tank",
          },
          {
            name: "Transformer Relay",
          },
          {
            name: "Transformer Bushing",
          },
          {
            name: "Transformer Core",
          },
          {
            name: "Transformer Winding",
          },
        ],
      },
    ],
  },
  {
    title: "Industrial Power System Products & Telecom Power System",
    isSubTitled: false,
    sub: [
      {
        name: "UPS",
      },
      {
        name: "Automatic Voltage Regulator (AVR)",
      },
      {
        name: "Automatic Battery Charger",
      },
      {
        name: "DC Power System (Battery Bank)",
      },
      {
        name: "LiFePO4 Battery System",
      },
      {
        name: "Rectifier System",
      },
      {
        name: "Inverter System",
      },
      {
        name: "Load Bank",
      },
      {
        name: "Power Management Monitoring System",
      },
      {
        name: "Power Distribution Unit (PDU)",
      },
      {
        name: "Busbar Trunking System (BBTS)",
      },
      {
        name: "Precision Air Conditioner",
      },
      {
        name: "Battery Monitoring System",
      },
      {
        name: "Spare Parts",
      },
    ],
  },
  {
    title: "Industrial Automation Products",
    isSubTitled: false,
    sub: [
      {
        name: "Feeder Protection Relay",
      },
      {
        name: "Bay Control & Protection Relay",
      },
      {
        name: "Transformer Control & Protection Relay",
      },
      {
        name: "Transmission Line Distance Protection Relay",
      },
      {
        name: "Motor Control & Protection Relay",
      },
      {
        name: "Generator Control & Protection Relay",
      },
      {
        name: "Diesel Generator Controller",
      },
      {
        name: "Capacitor Bank Protection Relay",
      },
      {
        name: "Overhead Line Protection Relay",
      },
      {
        name: "Tap Changer Protection Relay",
      },
      {
        name: "Power Management Relay",
      },
      {
        name: "IDMT Relay",
      },
      {
        name: "Recloser Control & Protection Relay",
      },
      {
        name: "Grid Automation Controller",
      },
      {
        name: "Distribution Automation Controller",
      },
      {
        name: "IEC 61850 Based Substation Automation System",
      },
      {
        name: "Programmable Logic Control(PLC)",
      },
      {
        name: "Programmable Automation Controller (PAC)",
      },
      {
        name: "Industrial Sensors",
      },
      {
        name: "Industrial Network Products",
      },
      {
        name: "Industrial Control Products",
      },
      {
        name: "HMI",
      },
      {
        name: "Industrial Panel Computer(IPC)",
      },
      {
        name: "Safety Components",
      },
      {
        name: "Safety Instrumented System",
      },
      {
        name: "Condition Monitoring and I/O",
      },
      {
        name: "Advanced Process Control Systems",
      },
      {
        name: "Manufacturing Execution System",
      },
      {
        name: "DCS System",
      },
      {
        name: "SCADA System",
      },
    ],
  },
  {
    title: "Security System Products",
    isSubTitled: false,
    sub: [
      {
        name: "CCTV",
      },
      {
        name: "IP Camera",
      },
      {
        name: "Network Video Recorder(NVR)",
      },
      {
        name: "Digital Video Recorder(DVR)",
      },
      {
        name: "Data Center(Server)",
      },
      {
        name: "Data Storage",
      },
      {
        name: "Video Wall",
      },
      {
        name: "Monitor",
      },
      {
        name: "Decoder",
      },
      {
        name: "Video Wall Controller",
      },
      {
        name: "Smart Surveillance System",
      },
      {
        name: "Access Control System Products",
      },
      {
        name: "Video Intercom Products",
      },
      {
        name: "Thermal Camera",
      },
      {
        name: "Alarm Products",
      },
      {
        name: "Enterprise Software",
      },
      {
        name: "Accessories",
      },
    ],
  },
  {
    title: "Telecommunication System",
    isSubTitled: false,
    sub: [
      {
        name: "Industrial Network Switch",
      },
      {
        name: "Router",
      },
      {
        name: "Networking Products",
      },
      {
        name: "Internet of Things(IoT) Products",
      },
      {
        name: "Mobility & Wireless Products",
      },
      {
        name: "Network Security Products",
      },
      {
        name: "Data Center Products",
      },
      {
        name: "Cloud",
      },
      {
        name: "Wireless AP/Bridge/Client",
      },
      {
        name: "Network Security Appliance",
      },
      {
        name: "Network Automation Products",
      },
      {
        name: "Network Management Software",
      },
      {
        name: "Software (Security, Data Center & Networking)",
      },
      {
        name: "Media Converter",
      },
      {
        name: "Serial Converter",
      },
      {
        name: "Protocol Gateway ",
      },
      {
        name: "SFP",
      },
    ],
  },
  {
    title: "Metering System",
    isSubTitled: false,
    sub: [
      {
        name: "Residential Meter",
      },
      {
        name: "Industrial and Commercial Meter",
      },
      {
        name: "HT CT Metering",
      },
      {
        name: "LTCT Meter",
      },
      {
        name: "Prepayment Meter",
      },
      {
        name: "Water Meter",
      },
      {
        name: "Gas Meter",
      },
    ],
  },
  {
    title: "Elevator System",
    isSubTitled: false,
    sub: [
      {
        name: "Passenger Elevator",
      },
      {
        name: "Observation Elevator",
      },
      {
        name: "Freight Elevator",
      },
      {
        name: "Car Elevator",
      },
      {
        name: "Villa Elevator",
      },
      {
        name: "Escalator",
      },
      {
        name: "Moving Walk",
      },
    ],
  },
  {
    title: "Protection & Control Panel",
    isSubTitled: false,
    sub: [
      {
        name: "Generator Control Panel (Natural Gas Generator/Diesel Generator/HFO Generator)",
      },
      {
        name: "Turbine Control & Protection Panel (Gas Turbine & Steam Turbine)",
      },
      {
        name: "Boiler Control & Protection Panel",
      },
      {
        name: "Air Insulated Switchgear (LV, MV & HV)",
      },
      {
        name: "Motor Control Center (LV, MV & HV)",
      },
      {
        name: "Gas Insulated Switchgear (MV & HV)",
      },
      {
        name: "Transformer Control & Protection Panel (MV & HV)",
      },
      {
        name: "Metering Panel (LV, MV & HV)",
      },
      {
        name: "Power Plant Control System",
      },
      {
        name: "Substation Control System",
      },
      {
        name: "Ring Main Unit (RMU) (MV & HV)",
      },
      {
        name: "Remote Terminal Unit (RTU) (MV & HV)",
      },
      {
        name: "Remote Terminal Unit (RTU) (MV & HV)",
      },
      {
        name: "Synchronization Panel",
      },
      {
        name: "Industrial Process Control System",
      },
    ],
  },
  {
    title: "Motor, Pump & Drive System",
    isSubTitled: false,
    sub: [
      {
        name: "AC Motor (LV, MV & HV)",
      },
      {
        name: "Induction Motor (LV, MV & HV)",
      },
      {
        name: "Synchronous Motor",
      },
      {
        name: "Radiator Motor",
      },
      {
        name: "DC Motor",
      },
      {
        name: "Servo Motor",
      },
      {
        name: "Brake Motor",
      },
      {
        name: "Geared Motor",
      },
      {
        name: "Gearless Motor",
      },
      {
        name: "Hydraulic Pump",
      },
      {
        name: "Centrifugal Pump",
      },
      {
        name: "Liner Cooling Pump",
      },
      {
        name: "Gear Oil Pump",
      },
      {
        name: "Industrial Exhaust Fan Unit",
      },
      {
        name: "Industrial Ventilation Fan Unit",
      },
      {
        name: "Soft Starter",
      },
      {
        name: "Star Delta Starter",
      },
      {
        name: "AC Drive (LV, MV & HV)",
      },
      {
        name: "DC Drive",
      },
      {
        name: "Servo Drive",
      },
    ],
  },
  {
    title: "Renewable Energy Generation Products",
    isSubTitled: true,
    sub: [
      {
        subTitle: "Solar Products",
        subOfSub: [
          {
            name: "Solar Panel (Poly Crystalline & Mono Crystalline)",
          },
          {
            name: "Solar Battery System",
          },
          {
            name: "Solar Charge Controller",
          },
          {
            name: "Solar Inverter (Off Grid & On Grid)",
          },
          {
            name: "Solar Camping Light",
          },
          {
            name: "Solar Torch",
          },
          {
            name: "Solar Charger",
          },
          {
            name: "Solar Fan",
          },
          {
            name: "Solar Street Light",
          },
          {
            name: "Solar Air Conditioner",
          },
          {
            name: "Solar Irrigation Pump",
          },
          {
            name: "Solar Lighting Kits",
          },
          {
            name: "Solar Refrigerator",
          },
          {
            name: "Solar TV",
          },
          {
            name: "Solar Garden Lamp",
          },
          {
            name: "Solar Lawn Lamp",
          },
          {
            name: "Solar Traffic Warning Light",
          },
          {
            name: "Wind Solar Hybrid Street Light",
          },
        ],
      },
      {
        subTitle: "Solar Power System",
        subOfSub: [
          {
            name: "Solar Home Power System (5W-2000W)",
          },
          {
            name: "Off Grid Solar Power System (600W-500KW)",
          },
          {
            name: "On Grid Solar Power System (1MW-50MW)",
          },
          {
            name: "Grid Tied Solar Power Plant",
          },
        ],
      },
      {
        subTitle: "Wind Products",
        subOfSub: [
          {
            name: "Wind Turbine",
          },
          {
            name: "Wind Battery Bank",
          },
          {
            name: "Wind Charge Controller",
          },
          {
            name: "Off grid Wind Inverter",
          },
          {
            name: "On grid Wind Inverter",
          },
          {
            name: "Hybrid Power System",
          },
        ],
      },
      {
        subTitle: "Wind Power System",
        subOfSub: [
          {
            name: "Wind Home Power System",
          },
          {
            name: "Off Grid Wind Power System",
          },
          {
            name: "On Grid Wind Power System",
          },
          {
            name: "Grid Tied Wind Power Plant",
          },
        ],
      },
      {
        subTitle: "Hybrid Power System",
        subOfSub: [
          {
            name: "Wind Solar Hybrid Power System",
          },
          {
            name: "Solar Diesel Hybrid Power System",
          },
          {
            name: "Wireless Solar Data Acquisition System",
          },
          {
            name: "Solar Mobile Charging System",
          },
          {
            name: "PV Desalination System",
          },
        ],
      },
    ],
  },
  {
    title: "Industrial Production Plant",
    isSubTitled: false,
    sub: [
      {
        name: "Industrial Water Treatment Plant",
      },
      {
        name: "Effluent Treatment Plant (ETP)",
      },
      {
        name: "Beverage Plant",
      },
      {
        name: "Auto Rice Mill",
      },
      {
        name: "Automatic Flour Mill",
      },
      {
        name: "Automatic Clay Brick Production Line",
      },
      {
        name: "Detergent Plant",
      },
      {
        name: "Steel Production Plant",
      },
      {
        name: "Sugar Production Plant",
      },
      {
        name: "Fertilizer Production Plant",
      },
      {
        name: "Other Production Plant",
      },
    ],
  },
  {
    title: "Material Handling Equipment",
    isSubTitled: false,
    sub: [
      {
        name: "Batcher Trolley",
      },
      {
        name: "Forklift and Forklift Attachment",
      },
      {
        name: "Pallet Truck",
      },
      {
        name: "Forklift Truck",
      },
      {
        name: "Counterbalance Truck",
      },
      {
        name: "Towing Tractor",
      },
      {
        name: "Component & Spare Parts",
      },
      {
        name: "Component & Spare Parts",
      },
    ],
  },
  {
    title: "Construction Machinery",
    isSubTitled: false,
    sub: [
      {
        name: "Excavator",
      },
      {
        name: "Backhoe",
      },
      {
        name: "Bulldozer",
      },
      {
        name: "Graders",
      },
      {
        name: "Wheel Tractor Scraper",
      },
      {
        name: "Trencher",
      },
      {
        name: "Wheel Loader",
      },
      {
        name: "Crane",
      },
      {
        name: "Compactor",
      },
      {
        name: "Pile Boring Machine",
      },
      {
        name: "Aerial Working Platform Truck",
      },
      {
        name: "Batching Plant",
      },
      {
        name: "Truck mounted concrete pump",
      },
      {
        name: "Concrete Mixer Machine",
      },
      {
        name: "Dump Truck",
      },
      {
        name: "Truck Mixer",
      },
      {
        name: "Hoist",
      },
      {
        name: "Stacker",
      },
      {
        name: "Road Construction Machinery",
      },
      {
        name: "Components and Spare Parts",
      },
    ],
  },
  {
    title: "Architect Finishing Materials",
    isSubTitled: true,
    sub: [
      {
        subTitle: "Tile",
        subOfSub: [
          {
            name: "Gun sprayed tile",
          },
          {
            name: "Vinyl composition tile (PVC)",
          },
          {
            name: "Homogeneous ceramic tile for floor",
          },
          {
            name: "Glazed ceramic tile for wall",
          },
          {
            name: "Acid resistance tile for floor",
          },
          {
            name: "Acid resistance tile for wall",
          },
          {
            name: "Vinyl base",
          },
          {
            name: "Anti-static PVC tile",
          },
          {
            name: "Carpet tile (wall to wall)",
          },
          {
            name: "Other Finishing Materials",
          },
        ],
      },
      {
        subTitle: "Paint",
        subOfSub: [
          {
            name: "Emulsion paint (int.)",
          },
          {
            name: "Emulsion paint (ext.)",
          },
          {
            name: "Epoxy paint (floor & wall)",
          },
          {
            name: "Acid resistance paint",
          },
          {
            name: "Chemical resistance paint",
          },
          {
            name: "Acrylic paint (int.)",
          },
        ],
      },
      {
        subTitle: "Ceiling",
        subOfSub: [
          {
            name: "Ceiling (PVC)",
          },
          {
            name: "Ceiling (aluminum)",
          },
          {
            name: "Ceiling (wool/mineral type)",
          },
          {
            name: "Gypsum board",
          },
        ],
      },
      {
        subTitle: "Door",
        subOfSub: [
          {
            name: "Fire steel door",
          },
          {
            name: "Normal steel door",
          },
          {
            name: "Wooden door",
          },
          {
            name: "Glass door",
          },
          {
            name: "PVC door",
          },
          {
            name: "Steel sliding door",
          },
          {
            name: "Tempered glass door",
          },
          {
            name: "Roll up door (steel shutter) ",
          },
        ],
      },
      {
        subTitle: "Other Finishing Materials",
        subOfSub: [
          {
            name: "Hardener",
          },
          {
            name: "Granolithic screed c/w floor hardener",
          },
          {
            name: "Hard screed topping carborundum type with epoxy coating",
          },
          {
            name: "Aluminum composite panel",
          },
          {
            name: "Window",
          },
          {
            name: "Sanitary ware",
          },
          {
            name: "Furniture",
          },
          {
            name: "FRP lining",
          },
          {
            name: "Built up waterproofing",
          },
          {
            name: "Waterstop (PVC)",
          },
          {
            name: "Liquid water proofing for floor",
          },
          {
            name: "Vapor barrier",
          },
          {
            name: "Liquid waterproofing",
          },
          {
            name: "Capillary waterproofing",
          },
          {
            name: "Aluminum louver",
          },
          {
            name: "Suspended noncombustible mineral fiber board ceiling",
          },
          {
            name: "STS window (double)",
          },
          {
            name: "Moisture resistant acoustic tile ceiling",
          },
          {
            name: "Powder coated noise reduction ceiling",
          },
        ],
      },
    ],
  },
  {
    title: "Other Products",
    sub: [],
  },
];
